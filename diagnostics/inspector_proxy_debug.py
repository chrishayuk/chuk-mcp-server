...
async def proxy_json_request(session, target_url, method, body, headers):
    ...
    try:
        ...
        try:
            async with session.get(f"{TARGET_SERVER}{TARGET_PATH}", timeout=aiohttp.ClientTimeout(total=5)) as test_response:
        ...
        async def sse_proxy_generator():
            try:
                ...
                async with aiohttp.ClientSession(timeout=timeout) as sse_session:
                    ...
                    print(f"\n🔍 SSE CONNECTION TEST:")
                    try:
                        async with sse_session.get(f"{TARGET_SERVER}{TARGET_PATH}", timeout=aiohttp.ClientTimeout(total=5)) as test_response:
                                    ...
                                    if line.strip():  # Non-empty lines
                                        ...
                                        if line.startswith('data: '):
                                            try:
                                                ...
                                                print(f"  📊 SSE Data JSON: {orjson.dumps(data_json, option=orjson.OPT_INDENT_2).decode()}")
                                                
                                                if "result" in data_json and isinstance(data_json["result"], dict):
                                                    if "sessionId" in data_json["result"] or "session_id" in data_json["result"]:
                                                        session_from_data = data_json["result"].get("sessionId") or data_json["result"].get("session_id")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int, 
        default=8011, 
        help="Port for the proxy server (default: 8011)"
    )
    parser.add_argument(
        "--server-port", 
        type=int, 
        default=8000,
        help="Port where your MCP server is running (default: 8000)"
    )
    parser.add_argument(
        "--server-host", 
        default="localhost",
        help="Host where your MCP server is running (default: localhost)"
    )
    parser.add_argument(
        "--server-path", 
        default="/mcp",
        help="Path to MCP endpoint on your server (default: /mcp)"
    )
