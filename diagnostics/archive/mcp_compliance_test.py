...
class MCPComplianceValidator:
    ...
    async def _test_utilities(self):
        ...
        # Test ping
        await self._test_case(
            "Ping Utility", "Server should respond to ping requests", self._test_ping, "Utilities - Ping"
        )
    ...
    async def _validate_jsonrpc_format(self) -> tuple[bool, str]:
        ...
        try:
            async with self.session.post(
                self.server_url, json=test_message, headers={"Content-Type": "application/json"}
            ) as resp:
                if resp.status != 200:
    ...
    async def _validate_required_fields(self) -> tuple[bool, str]:
        ...
        try:
            async with self.session.post(
                self.server_url, json=invalid_message, headers={"Content-Type": "application/json"}
            ) as resp:
                data = await resp.json()
    ...
    async def _validate_request_ids(self) -> tuple[bool, str]:
        ...
        invalid_message = {"jsonrpc": "2.0", "id": None, "method": "ping", "params": {}}
    ...
    async def _test_initialize(self) -> tuple[bool, str]:
        ...
        try:
            async with self.session.post(
                self.server_url, json=init_message, headers={"Content-Type": "application/json"}
            ) as resp:
                if resp.status != 200:
                    return False, f"HTTP status {resp.status}, expected 200"
    ...
    async def _test_initialized_notification(self) -> tuple[bool, str]:
        ...
        try:
            async with self.session.post(self.server_url, json=notification, headers=headers) as resp:
    ...
    async def _test_session_management(self) -> tuple[bool, str]:
        ...
        ping_message = {"jsonrpc": "2.0", "id": "session-test", "method": "ping", "params": {}}

        headers = {"Content-Type": "application/json", "Mcp-Session-Id": self.session_id}

        try:
            async with self.session.post(self.server_url, json=ping_message, headers=headers) as resp:
                if resp.status == 200:
    ...
    async def _test_tools_list(self) -> tuple[bool, str]:
        ...
        try:
            async with self.session.post(self.server_url, json=message, headers=headers) as resp:
                if resp.status != 200:
                    return False, f"HTTP status {resp.status}, expected 200"
    ...
    async def _test_tool_execution(self) -> tuple[bool, str]:
        ...
        message = {
            ...
            "id": "tool-call-test",
            "method": "tools/call",
            "params": {"name": test_tool["name"], "arguments": tool_args},
        }
    ...
    async def _test_error_security(self) -> tuple[bool, str]:
        ...
        invalid_message = {"jsonrpc": "2.0", "id": "error-test", "method": "nonexistent/method", "params": {}}
    ...
    async def _test_malformed_requests(self) -> tuple[bool, str]:
        ...
        try:
            async with self.session.post(
                self.server_url, data="invalid json {", headers={"Content-Type": "application/json"}
            ) as resp:
                if resp.status == 400:
    ...
    async def _test_method_not_found(self) -> tuple[bool, str]:
        ...
        try:
            async with self.session.post(
                self.server_url, json=message, headers={"Content-Type": "application/json"}
            ) as resp:
                data = await resp.json()
    ...
    async def _test_invalid_params(self) -> tuple[bool, str]:
        ...
        message = {
            ...
            "method": "tools/call",
            "params": {"name": test_tool["name"], "arguments": {"completely": "wrong", "parameters": 123}},
        }
    ...
    async def _test_meta_field_support(self) -> tuple[bool, str]:
        ...
        message = {
            ...
            "method": "ping",
            "params": {},
            "_meta": {"test": "compliance-validator"},
        }
    ...
    async def _test_case(self, name: str, description: str, test_func, spec_ref: str):
        ...
        try:
            ...
            test_case = TestCase(
                name=name, description=description, result=result, details=details, spec_reference=spec_ref
            )
    ...
    def _add_result(self, name: str, details: str, result: TestResult):
        ...
        test_case = TestCase(name=name, description="", result=result, details=details)
