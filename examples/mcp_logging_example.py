...
mcp = LoggingEnabledMCPServer(
    name="MCP Logging Demo Server",
    version="1.0.0",
    transport="stdio",
    debug=False
)
...
async def process_data(data: List[int], operation: str = "sum") -> Dict[str, Any]:
    ...
    mcp.log_server_event("PROCESSING_START", f"Processing {len(data)} items", {
        "operation": operation,
        "data_size": len(data)
    })
    ...
    try:
        ...
        mcp.log_server_event("PROCESSING_COMPLETE", f"Operation {operation} completed", {
            "result": result,
        ...
        mcp.log_server_event("PROCESSING_ERROR", f"Processing failed: {str(e)}", {
            "operation": operation,
            "error": str(e)
        })
...
async def simulate_long_task(duration: float = 2.0, fail_probability: float = 0.0) -> Dict[str, Any]:
    ...
    mcp.log_server_event("TASK_START", f"Starting task {task_id}", {
        "task_id": task_id,
        "duration": duration,
    ...
    try:
        ...
        for step in range(1, steps + 1):
            ...
            mcp.log_server_event("TASK_PROGRESS", f"Task {task_id} at {progress:.1f}%", {
                "task_id": task_id,
                "progress": progress,
                "step": step,
                "total_steps": steps
        ...
        mcp.log_server_event("TASK_COMPLETE", f"Task {task_id} completed", {
            "task_id": task_id,
            "duration": duration,
            "status": "success"
        })
        ...
        mcp.log_server_event("TASK_FAILED", f"Task {task_id} failed", {
            "task_id": task_id,
            "error": str(e),
            "status": "failed"
        })
...
def test_log_levels() -> Dict[str, str]:
    ...
    mcp.log_server_event("LOG_TEST", "Tested all log levels", {
...
if __name__ == "__main__":
    ...
    # Log server startup
    mcp.log_server_event("SERVER_START", "MCP Logging Demo Server starting up", {
