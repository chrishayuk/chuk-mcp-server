...
class TestSmartConfig:
    ...
    def test_get_performance_mode_with_environment(self):
        ...
        with patch.object(config.environment_detector, 'detect', return_value="serverless"):
            with patch.object(config.system_detector, 'detect_performance_mode', return_value="serverless_optimized") as mock_detect:
                        ...
                        with patch.object(config.system_detector, 'detect_optimal_workers', return_value=4):
                            with patch.object(config.system_detector, 'detect_max_connections', return_value=5000):
                                with patch.object(config.system_detector, 'detect_debug_mode', return_value=False):
                                    with patch.object(config.system_detector, 'detect_log_level', return_value="WARNING"):
                                        with patch.object(config.system_detector, 'detect_performance_mode', return_value="high_performance"):
                                            
                                            result = config.get_all_defaults()
                                            
                                            # Verify all expected keys are present
                                            expected_keys = {
                                                "project_name", "environment", "host", "port", "debug",
                                                "workers", "max_connections", "log_level", "performance_mode", "containerized"
                                            }
    ...
    def test_detect_all_method(self):
        ...
        with patch.object(config.project_detector, 'detect', return_value="Test Project"):
            with patch.object(config.environment_detector, 'detect', return_value="development"):
                with patch.object(config.container_detector, 'detect', return_value=False):
                    with patch.object(config.network_detector, 'detect_network_config', return_value=("localhost", 8000)):
                            ...
                            with patch.object(config.system_detector, 'detect_max_connections', return_value=1000):
                                with patch.object(config.system_detector, 'detect_debug_mode', return_value=True):
                                    with patch.object(config.system_detector, 'detect_log_level', return_value="INFO"):
                                        with patch.object(config.system_detector, 'detect_performance_mode', return_value="development"):
...
class TestSmartConfigIntegration:
    ...
    @patch('socket.socket')
    def test_production_docker_scenario(self, mock_socket, mock_memory, mock_cpu_count, 
                                       mock_exists, mock_read_text, mock_cwd):
    ...
    @patch('psutil.virtual_memory')
    @patch('socket.socket')
    def test_development_local_scenario(self, mock_socket, mock_memory, mock_cpu_count, 
