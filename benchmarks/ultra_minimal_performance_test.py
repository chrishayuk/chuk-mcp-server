...
class UltraMinimalPerfTest:
    ...
    async def run_ultra_minimal_tests(self):
        ...
        for concurrency in [10, 50, 100, 200, 500]:
            ...
            print(f"   {concurrency:>3} conn: {result.rps:>8,.0f} RPS | {result.avg_ms:>6.2f}ms avg | {result.success_rate:>5.1f}% success")
        ...
        print(f"   200 conn: {version_result.rps:>8,.0f} RPS | {version_result.avg_ms:>6.2f}ms avg | {version_result.success_rate:>5.1f}% success")
        ...
        print(f"   200 conn: {health_result.rps:>8,.0f} RPS | {health_result.avg_ms:>6.2f}ms avg | {health_result.success_rate:>5.1f}% success")
    ...
    async def _find_maximum_throughput(self) -> UltraResult:
        ...
        for concurrency in [50, 100, 200, 500, 1000, 1500, 2000]:
            ...
            result = await self._test_raw_http_endpoint(
                self.ping_request, f"Max Test ({concurrency})", 
