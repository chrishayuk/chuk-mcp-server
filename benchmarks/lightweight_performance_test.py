...
class LightweightPerfTest:
    ...
    def __init__(self, base_url: str = "http://localhost:8001"):
        ...
        self.tools_list_request = json.dumps({
            "jsonrpc": "2.0", 
            "id": 2,
            "method": "tools/list",
            "params": {}
    ...
    async def run_lightweight_tests(self):
        ...
        async with httpx.AsyncClient(
            ...
            ping_result = await self._test_http_endpoint(
                client, "/ping", "HTTP Ping", duration=5.0, concurrency=500
            )
            ...
            print(f"   {version_result.rps:>8,.0f} RPS | {version_result.avg_ms:>6.2f}ms avg | {version_result.success_rate:>5.1f}% success")
            ...
            print(f"   {health_result.rps:>8,.0f} RPS | {health_result.avg_ms:>6.2f}ms avg | {health_result.success_rate:>5.1f}% success")
            
            # Test 4: MCP Ping (JSON-RPC)
            if self.session_id:
                print("🎯 Testing MCP Ping (JSON-RPC)...")
                mcp_ping_result = await self._test_mcp_ping(
                    client, duration=5.0, concurrency=200
                )
                results.append(mcp_ping_result)
                print(f"   {mcp_ping_result.rps:>8,.0f} RPS | {mcp_ping_result.avg_ms:>6.2f}ms avg | {mcp_ping_result.success_rate:>5.1f}% success")
                
                # Test 5: MCP Tools List
                print("🔧 Testing MCP Tools List...")
                tools_result = await self._test_mcp_tools_list(
                    client, duration=5.0, concurrency=200
                )
            ...
            print(f"   {burst_result.rps:>8,.0f} RPS | {burst_result.avg_ms:>6.2f}ms avg | {burst_result.success_rate:>5.1f}% success")
    ...
    async def _test_mcp_ping(
        ...
        } if self.session_id else {'Content-Type': 'application/json'}
    ...
    async def _test_async_tools(self, client: httpx.AsyncClient):
        ...
        async_hello_msg = json.dumps({
            ...
            "params": {
                "name": "async_hello",
                "arguments": {"name": "PerfTest", "delay": 0.01}
            }
        }).encode('utf-8')
        ...
        result = await self._test_mcp_request(client, concurrent_msg, "Concurrent Requests", concurrency=50, duration=3.0)
        print(f"      {result.rps:>8,.0f} RPS | {result.avg_ms:>6.2f}ms avg | {result.success_rate:>5.1f}% success")
    ...
    def _print_performance_summary(self, results: List[PerfResult]):
        ...
        for result in results:
            print(f"   {result.name:<18} | {result.rps:>8,.0f} | {result.avg_ms:>6.1f} | "
