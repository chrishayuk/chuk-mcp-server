...
class BasicHTTPTest:
    ...
    async def _test_individual_endpoints(self):
        ...
        for endpoint in endpoints:
            ...
            # Performance test
            result = await self._test_single_endpoint(endpoint["path"], endpoint["name"], concurrency=10, duration=3.0)
            ...
            if result:
                ...
                print(f"   {status}")
                print(
                    f"   📊 {result.rps:>8.1f} RPS | {result.avg_ms:>6.2f}ms avg | {result.success_rate:>5.1f}% success"
                )
    ...
    async def _test_concurrency_levels(self):
        ...
        for concurrency in concurrency_levels:
            ...
            if result:
                ...
                print(
                    f"{concurrency:>10} | {result.rps:>8.1f} | {result.avg_ms:>6.1f} | "
